...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $params = [
                ...
                'rel_behavior' => isset($this->rel_behaviator[$tableName]) ? $this->rel_behaviator[$tableName] : [],
            ];
            $files[] = new CodeFile(
    ...
    private function generateManyManyRelations($table, $fks, $relations)
    {
        ...
        foreach ($fks as $pair) {
            ...
            $this->genereteManyManyBehaviator($className1, $table->name, $table0Schema->fullName, $relationName);
            ...
            $this->genereteManyManyBehaviator($className0, $table->name, $table1Schema->fullName, $relationName);
        }
    ...
    public function genereteManyManyBehaviator($className, $table, $tableFullName, $relationName) {

        $result = [];

        $result[] = yii\helpers\Inflector::variablize($relationName);

        $rel_function_list  = "";
        $rel_function_list .= "    /**\n";
        $rel_function_list .= "     * @return array\n";
        $rel_function_list .= "     */\n";
        $rel_function_list .= "    public function get".$relationName."List()\n";
        $rel_function_list .= "    {\n";
        $rel_function_list .= "        return \yii\helpers\ArrayHelper::map(\n";
        $rel_function_list .= "            $className::find()->active()->all(),\n";
        $rel_function_list .= "            'id',\n";
        $rel_function_list .= "            'title'\n";
        $rel_function_list .= "        );\n";
        $rel_function_list .= "    }\n";

        $result[] = $rel_function_list;

        $this->rel_behaviator[$tableFullName][$relationName] = $result;
    }
